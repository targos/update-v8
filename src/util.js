...
exports.getNodeV8Version = function getNodeV8Version(cwd) {
    try {
        ...
        const major = parseInt(/V8_MAJOR_VERSION (\d+)/.exec(v8VersionH)[1]);
        const minor = parseInt(/V8_MINOR_VERSION (\d+)/.exec(v8VersionH)[1]);
        const build = parseInt(/V8_BUILD_NUMBER (\d+)/.exec(v8VersionH)[1]);
        const patch = parseInt(/V8_PATCH_LEVEL (\d+)/.exec(v8VersionH)[1]);
